...
def _in_fov(vehicle: VehicleLike, point: tuple[float, float], fov_deg: float,
            range_cm: float = FIXED_RULES.perception_range_cm) -> bool:
...
class SimulatorAutonomousAdapter:
    ...
    def _input(self,vehicle:VehicleLike,obstacles:list[ObstacleLike])->PlannerInput:
        ...
        for i, obstacle in enumerate(obstacles, 1):
            if _in_fov(vehicle, (obstacle.x, obstacle.y), self.fov_deg):
                # PASSED solo puede comenzar después de una detección real
                # dentro del FOV frontal del vehículo.
                if not obstacle.passed:
                    obstacle.ever_detected = True
                visible_items.append(VisibleObstacle(
                    str(i), obstacle.x, obstacle.y, self.obstacle_size_cm,
                    self.obstacle_size_cm, obstacle.color, 0.0, obstacle.passed,
                ))
    ...
    def update(self,vehicle:VehicleLike,obstacles:list[ObstacleLike],dt:float)->None:
        ...
        if self.latest_result is not None and self.elapsed_s<self.next_replan_s:
            ...
            self.next_replan_s = self.elapsed_s + self.controller.execution_interval_s(
                self.latest_result.best_candidate.trajectory_points[0].state
                if self.latest_result.best_candidate and self.latest_result.best_candidate.trajectory_points
                else VehicleState(vehicle.x, vehicle.y, vehicle.heading, vehicle.speed_cm_s),
                command.target_speed_cm_s,
            )
    ...
    def decision_probe(self, vehicle: VehicleLike, obstacles: list[ObstacleLike]) -> PlannerResult:
        """Recalcula todos los candidatos sin commitment ni movimiento."""
        # El probe es una inspección independiente: debe mostrar siempre el
        # conjunto completo de candidatos de la pose actual, no solamente la
        # trayectoria que AutonomousController tendría comprometida.
        result = self.planner.plan(self._input(vehicle, obstacles))
        self.latest_result = result
        self.planner_state = result.state.value
        self.planning_phase = result.state.value
        self.state = (
            AvoidState.EMERGENCY_STOP
            if result.state is PlannerState.NO_SAFE_TRAJECTORY
            else AvoidState.FOLLOW
        )
        return result
